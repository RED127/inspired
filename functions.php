...
function get_build_amount()
{
    ...
    $query = "SELECT value FROM {$tblLive} WHERE `name` = '{$tag}' ORDER BY id DESC LIMIT 1";
    $result = $db->query($query);
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        foreach ($parts as $part) {
            ...
            echo '<td style="text-align: center;">
                    <div class="row text-center justify-content-center">
                        <input type="text" class="form-control mr-2" id="low" name="low" placeholder="LOW" style="width:70px" readonly value="' . $part->level_low . '">
                        <input type="text" class="form-control" id="medium" name="medium" placeholder="MEDIUM" style="width:100px" readonly value="' . $part->level_medium . '">
...
function save_part($post_data)
{
    ...
    if ($part_id == 0) {
        ...
        $query = "UPDATE {$tblParts} SET `part_no` = '{$part_no}', `part_name` = '{$part_name}', `amount` = '{$amount}', `sf` = '{$sf}', `ps` = '{$ps}', `fl` = '{$fl}', `level_low` = '{$level_low}', `level_medium` = '{$level_medium}' WHERE `id` = {$part_id}";
    }
    $result = $db->query($query);
    if ($result)
        echo 'Ok';
    ...
    foreach ($part_barcodes as $index => $part_barcode) {
        ...
        if ($data['error'] == '') {
            if ($_SESSION['stocking_action'] == "out") {
                if ($flag)
                    $data['success'] = 'Part : ' . $part['part_no'] . '(' . $part['part_no'] . ') has been scanned out ' . $row['area'] . ', ' . $lane_barcode;
                else
                    $data['success'] = 'Part : ' . $part['part_no'] . '(' . $part['part_no'] . ') has been scanned out ' . $row['area'] . ', Lane' . $row['lane_no'];
            } else {
                if ($flag)
                    $data['success'] = 'Part : ' . $part['part_no'] . '(' . $part['part_no'] . ') has been scanned to ' . $row['area'] . ', ' . $lane_barcode;
                else
                    $data['success'] = 'Part : ' . $part['part_no'] . '(' . $part['part_no'] . ') has been scanned to ' . $row['area'] . ', ' . $row['lane_no'];
            }
        }
    }
...
function set_help_alarm($post_data)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql = "INSERT INTO {$tblHelpAlarm}  (`user_id`, `clicked_time`, `is_confirm`, `page`) values ('{$user_id}', NOW(), 0, '{$page}') ";
        $db->query($sql);
...
function confirm_help_alarm($post_data)
{
    ...
    $query = "UPDATE {$tblHelpAlarm} SET is_confirm = 1, turnoff_time = NOW() WHERE id = {$alarm_id}";
    $db->query($query);
}
...
function get_container_devan($start, $end)
{
    ...
    $query = "SELECT * FROM {$tblContainerDevan} WHERE `date` BETWEEN  '{$start}' AND '{$end}' ORDER BY `date` ASC, `time` ASC";
    $result = $db->query($query);
    $items = array();
...
function get_live_deban($post_data)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($devan_plan_flag) {
            $devan_plan_value = rtrim(explode("-", $devan_plan)[0]);
            echo '<h1 style="font-size: 38px;text-align: center;">' . $devan_plan_value . '</h1>';
            $complete_btn_disabled = "";
        }
        ...
        echo '<h1 style="font-size: 32px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Current Devan: </p>' .  '<span style="margin-right: 20px; text-transform: uppercase;font-weight:bold">' . date('d/m/Y', strtotime($devan['date'])) . '</span><span style="margin-right: 20px; text-transform: uppercase;font-weight:bold">' . $shift . '</span><span style="font-weight:bold">' . $devan['time'] . '</span></h1>';
        echo '</div>';
        echo '<div class="col-md-4">';
        echo '<h1 style="font-size: 32px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Member: </p>' .  '<span style="margin-right: 0px; text-transform: uppercase;">' . $user . '</span></h1>';
        echo '</div>';
        echo '</div>';
        //Container Renban
        //if north america member dose not need to confirm container number to press finish
        ...
        echo '<div class="col-md-8">';
        echo '<h1 style="font-size: 32px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Container No: </p><span style="color: white; font-size: 48px; font-weight:bold">' . $devan['on_doc_container_number'] . '</span></h1>';
        echo '</div>';
        $revan_state = $devan['revan_state'] == "scheduled" ? "Yes" : "No";
        echo '<div class="col-md-4">';
        echo '<h1 style="font-size: 32px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Revan: </p>' .  '<span style="margin-right: 20px; text-transform: uppercase; font-size: 48px;font-weight:bold">' . $revan_state . '</span></h1>';
        echo '</div>';
        echo '</div>';
...
function get_live_deban_overview1($post_data)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<h1 style="font-size: 22px;"><p style="color: gray; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Current Devan: </p>' .  '<span style="margin-right: 20px; text-transform: uppercase;font-weight:bold">' . date('d/m/Y', strtotime($devan['date'])) . '</span><span style="margin-right: 20px; text-transform: uppercase;font-weight:bold">' . $shift . '</span><span style="font-weight:bold">' . $devan['time'] . '</span></h1>';


        echo '<div class="mx-auto my-2">';
        echo '<h1 style="font-size: 22px;"><p style="color: gray; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Container No: </p><span style="color: black; font-size: 38px; font-weight:bold">' . $devan['on_doc_container_number'] . '</span></h1>';
        echo '</div>';

        echo '<div class="col-md-4 d-none">';
        echo '<h1 style="font-size: 22px;"><p style="color: gray; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Member: </p>' .  '<span style="margin-right: 0px; text-transform: uppercase;">' . $user . '</span></h1>';
        echo '</div>';
        echo '</div>';
        //Container Renban
        //if north america member dose not need to confirm container number to press finish
        ...
        echo '<h1 style="font-size: 22px;">
        <p style="color: gray; text-transform: uppercase; font-weight:bold; margin-bottom:auto" >Revan: </p>' .  '<span style="margin-right: 20px; text-transform: uppercase; font-size: 60px;font-weight:bold">' . $revan_state . '</span></h1>';
        echo '</div>';
        echo '</div>';
...
function get_live_deban_team_leader($post_data)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<div class="row mx-0">';
        echo '<div class="col-md-8">';
        echo '<h1 style="font-size: 18px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Current Devan: </p>' .  '<span style="margin-right: 0px; text-transform: uppercase;font-weight:bold">' . date('d/m/Y', strtotime($devan['date'])) . '</span><span style="margin-right: 20px; text-transform: uppercase;font-weight:bold">' . $shift . '</span><span style="font-weight:bold">' . $devan['time'] . '</span></h1>';
        echo '</div>';
        echo '<div class="col-md-4" style="margin-left: 0px;">';
        echo '<h1 style="font-size: 18px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto;">Member: </p>' .  '<span id="member_name" style="margin-right: 0px; text-transform: uppercase;">' . $user . '</span></h1>';
        echo '</div>';
        echo '</div>';
        //Container Renban
        //if north america member dose not need to confirm container number to press finish
        ...
        echo '<div class="row mt-4 mx-0">';
        echo '<div class="col-md-8">';
        echo '<h1 style="font-size: 18px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Container No: </p><span style="color: white; font-size: 28px; font-weight:bold">' . $devan['on_doc_container_number'] . '</span></h1>';
        echo '</div>';
        $revan_state = $devan['revan_state'] == "scheduled" ? "Yes" : "No";
        echo '<div class="col-md-4">';
        echo '<h1 style="font-size: 18px;"><p style="color: black; text-transform: uppercase; font-weight:bold; margin-bottom:auto">Revan: </p>' .  '<span style="margin-left: 20px; text-transform: uppercase; font-size: 28px;font-weight:bold">' . $revan_state . '</span></h1>';
        echo '</div>';
        echo '</div>';
...
{ // direction : in/out
    ...
    // if ($direction == "out")
    {
        if ($stock_available_lanes)
            $sort_query = "SELECT id, part, page, lane_id, lane_id_fl FROM {$tblScanLog} WHERE `lane_id` IN (" . implode(',', $stock_available_lanes) . ")" . " AND booked_out_time IS NULL AND LCASE(part)='" . strtolower($part_no) . "' ORDER BY booked_in_time LIMIT 1";
        else
            return $return_html;
    ...
    foreach ($filled_lanes as $lane_id) {
        ...
        while ($row = mysqli_fetch_object($result)) {
            if ($lane_inf->area == 'Free Location') {
                ...
                $amount += $part['amount'];
            } else if (strtoupper($row->part) == strtoupper($part['part_no'])) {
...
{
    global $db, $tblConveyancePicks, $tblDolly, $tblOPRSetting;
    ...
    // if($status == 'delivery')
    // {
    //     $query = "SELECT distinct kanban as dolly FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' ORDER BY kanban";
    // } else
    {
        $query = "SELECT DISTINCT pk.dolly FROM part_to_kanban pk INNER JOIN {$tblConveyancePicks} cp ON cp.kanban_date='{$pick_date}' AND cp.kanban = pk.kanban AND LENGTH(pk.dolly) > 0 AND cp.cycle={$cur} ORDER BY pk.dolly";
        $result = $db->query($query);
    ...
    if ($max_cycle) {
        ...
        if ($kanban_id == 0) {
            if ($status == 'pick') {
                ...
                $error = '';
                // if ($post_data['cycle'] == -1)
                // $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_completed=0 AND kanban_date = '{$pick_date}' ORDER BY `is_help` ASC, `cycle` ASC, `is_completed` ASC, `imported_at` ASC, `kanban` ASC LIMIT 1";
                // else
                // $query = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `kanban` ASC LIMIT 1";
                ...
                if ($zone != 'All') //Help or Other filters
                    ...
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle=1 ORDER BY  `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `pick_seq` ASC, `kanban` ASC";
                // $query = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `kanban` ASC LIMIT 1";
                ...
                $result = $db->query($query);
                $num_rows = mysqli_num_rows($result);
                // if ($num_rows == 0 && $post_data['cycle'] == -1) {
                ...
                $error = '';
                if ($post_data['cycle'] == -1)
                    // $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered=0 AND kanban_date = '{$pick_date}' ORDER BY `is_help` ASC, `is_delivered` ASC, `imported_at` ASC, `kanban` ASC";
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered=0 AND kanban_date = '{$pick_date}' AND cycle=1 ORDER BY `is_help` ASC, `is_delivered` ASC, `imported_at` ASC, `kanban` ASC";
                else
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered=0 AND kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_delivered` ASC, `imported_at` ASC, `kanban` ASC";
        ...
        $tmp = $db->query("SELECT * FROM {$tblDolly}");
        $dolly_colors = mysqli_fetch_all($tmp, MYSQLI_ASSOC);
        $dolly_colors = array_combine(array_column($dolly_colors, "name"), array_column($dolly_colors, "color"));
        if ($num_rows > 0) {
            $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
            if ($status == 'delivery') {
                $arr = ["SPS2", "ASSY1", "ASSY2", "SB1", "SB2", "ASSY3", "HS1", "KARAKURI", "CH2", "SPS3", "SPS4"];
                usort($res, function ($a, $b) use ($arr) {
                    ...
                    foreach ($arr as $index => $each_arr) {
                        ...
                        if (strpos($a['delivery_address'], $each_arr) !== false)
                            $ind1 = $index;
                        if (strpos($b['delivery_address'], $each_arr) !== false)
                            $ind2 = $index;
                    }
                    if ($ind1 > $ind2)
                        return 1;
            ...
            $dolly = get_dolly_by_name($kanban['kanban']);
            // $data['cur_zone'] = $kanban['kanban'];
            if ($dolly) {
                $data['dolly'] = $dolly->dolly;
                if (isset($dolly_colors[$data['dolly']]))
                    $data['dolly_color'] = $dolly_colors[$data['dolly']];
            }
            $_SESSION['overview_pick_pick'] = $data['pick_status'];
            $_SESSION['overview_pick_zone'] = $zone;
            $_SESSION['overview_pick_cycle'] = $data['cur_cycle'] . '/' . $data['max_cycle'];
            $_SESSION['overview_pick_kanban'] = $data['kanban'];
            $_SESSION['overview_pick_user'] = $_SESSION['user']['username'];
            $_SESSION['overview_pick_OPR'] = ceil($total / get_opr_setting('pick') * 100);

            $_SESSION['overview_delivery_pick'] = $data['pick_status'];
            $_SESSION['overview_delivery_zone'] = $data['dolly'];
            $_SESSION['overview_delivery_cycle'] = $data['cur_cycle'] . '/' . $data['max_cycle'];
            ...
            $_SESSION['overview_delivery_OPR'] = ceil($total / get_opr_setting('delivery') * 100);
...
function get_pick_address_by_kanban_and_delivery($kanban, $delivery_address)
{
    ...
    // $query = "SELECT * FROM {$tblPart2Kanban} WHERE kanban LIKE '{$kanban}%' AND (REPLACE(REPLACE(delivery_address, '-', ''), '.' ,'') = '{$d_address}' OR delivery_address2 = '{$delivery_address}')";
...
{
    global $db, $tblConveyancePicks, $tblDolly;
    $pick_date = convert_date_string($post_data['pick_date']);
    ...
    if (isset($post_data['zone']))
        $zone = $post_data['zone'];
    // if ($status == 'delivery') {
    //     $query = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle='{$post_data['cycle']}' ORDER BY  `is_help` ASC, `is_completed` ASC, `is_delivered` ASC, `imported_at` ASC";
    // }
    // else
    {
...
function make_pick_list_tr_first($pick, $status)
{
    ...
    echo '<td class="select-kanban" style="text-align: center; vertical-align: middle; font-size: x-large; color: white; background-color: ' . $pick['color'] . '"  data-kanban="' . $pick['id'] . '">' . strtoupper($pick['dolly']) . '<p class="cycle">CYCLE ' . $pick['cycle'] . '</p></td>';
    echo '</td>';
    echo '</tr>';
}
...
function conveyance_pick($post_data)
{
    ...
    if ($kanban_id) {
        ...
        if ($post_data['is_pick'] == "1") {
            $query = "UPDATE {$tblConveyancePicks} SET is_completed = 1, picked_at ='{$current}', completed_reason = {$reason} WHERE id = {$kanban_id}";
        } else {
            $query = "UPDATE {$tblConveyancePicks} SET is_delivered = 1, deliveried_at ='{$current}', completed_reason = {$reason} WHERE id = {$kanban_id}";
        }
        // echo $query;
        $result = $db->query($query);
        if ($result)
...
function check_pick_finish($post_data)
{
    ...
    if ($status == 'pick') {
        if ($post_data['cycle'] == -1)
            ...
            // var_dump()
            if ($post_data['zone'] == -1)
                $query = "SELECT * FROM {$tblConveyancePicks} WHERE cycle={$post_data['cycle']} AND is_completed = 0 AND kanban_date = '{$pick_date}'";
            else
                $query = "SELECT * FROM {$tblConveyancePicks} WHERE cycle='{$post_data['cycle']}' AND dolly='{$post_data['zone']}' AND is_completed = 0 AND kanban_date = '{$pick_date}'";

        $result = $db->query($query);
        $num = mysqli_num_rows($result);
        var_dump($num);
        if ($num == 0) {
            if ($post_data['cycle'] == $max_cycle || $post_data['cycle'] == -1)
                echo 'finish';
            else
                echo 'success';
        } else {
            $query1 = "SELECT * FROM {$tblConveyancePicks} WHERE cycle={$post_data['cycle']} AND is_completed = 0 AND is_help = 1 AND kanban_date = '{$pick_date}' AND dolly='{$post_data['zone']}'";
        ...
        $query = "SELECT * FROM {$tblConveyancePicks} WHERE cycle={$post_data['cycle']} AND is_delivered = 0 AND kanban_date='{$pick_date}'";
        $result = $db->query($query);
        $num = mysqli_num_rows($result);
        if ($num == 0) {
            if ($post_data['cycle'] == $max_cycle || $post_data['cycle'] == -1) {
            ...
            $query = "SELECT * FROM {$tblConveyancePicks} WHERE cycle={$post_data['cycle']} AND is_completed = 1 AND is_delivered = 0 AND is_help = 1 AND kanban_date='{$pick_date}'";
            $result = $db->query($query);
            $num = mysqli_num_rows($result);
            if ($num == 0)
                echo 'in_progress';
...
function conveyance_andon_help($post_data)
{
    ...
    if ($kanban_id) {
        if ($status == 'pick')
            $query = "UPDATE {$tblConveyancePicks} SET is_completed = 0,is_pick = 1, is_help = 1, helped_user = '{$user}', helped_at = '{$today}' WHERE id = {$kanban_id}";
        else
            $query = "UPDATE {$tblConveyancePicks} SET is_delivered = 0,is_pick = 0, is_help = 1, helped_user = '{$user}', helped_at = '{$today}' WHERE id = {$kanban_id}";
        $result = $db->query($query);
        if ($result)
            echo 'ok';
        else
...
{
    global $tblConveyancePicks, $tblHelpAlarm, $db;
    $kanban_id = $post_data['kanban_id'];
    ...
    if ($kanban_id) {
        if (isset($post_data['is_completed']))
            $query = "UPDATE {$tblConveyancePicks} SET picked_at = '{$post_data['today']}', is_completed = 1, completed_reason = {$reason} WHERE id = {$kanban_id}";
        if (isset($post_data['is_delivered']))
            $query = "UPDATE {$tblConveyancePicks} SET deliveried_at = '{$post_data['today']}', is_delivered = 1, delivered_reason = {$reason} WHERE id = {$kanban_id}";
        $result = $db->query($query);
        if ($result)
            echo 'ok';
        else
...
function get_dolly_by_name($name)
{
    ...
    $query = "SELECT * FROM {$tblPart2Kanban} WHERE kanban = '{$name}' LIMIT 1";
    $result = $db->query($query);
...
{
    global $db, $tblPart2Kanban;
    $query = "SELECT * FROM {$tblPart2Kanban} WHERE kanban = '{$name}' AND delivery_address = '{$delivery}' LIMIT 1";
    $result = $db->query($query);
    $dolly = mysqli_fetch_object($result);
    return $dolly;
}
...
function save_reason($post_data)
{
    ...
    if ($reason_id == 0) {
        ...
        echo '<td style="text-align: center;"><button type="button" class="btn btn-danger delete-reason" value="' . $insert_id . '">Delete</button></td>';
        echo '</tr>';
    } else {
        $query = "UPDATE {$tblReason} SET `name` = '{$reason_name}' WHERE `id` = {$reason_id}";
        $result = $db->query($query);
        if ($result)
            echo 'Ok';
        else
...
function update_reason($post_data)
{
    ...
    $field = $post_data['field'];
    $value = $post_data['value'];
    $update_query = "UPDATE {$tblReason} SET `{$field}` = '{$value}' WHERE id = {$reason_id}";
    $result = $db->query($update_query);
    if ($result)
        echo 'Success';
    else
...
function save_part2kanban($post_data)
{
    ...
    if ($item_id == 0) {
        ...
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control dolly" name="dolly" value="' . $dolly . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control barcode" name="barcode" value="' . $barcode . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control pick_address" name="pick_address" value="' . $pick_address . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control delivery_address" name="delivery_address" value="' . $delivery_address . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control delivery_address2" name="delivery_address2" value="' . $delivery_address2 . '">';
        echo '</td>';
        echo '<td>';
...
function read_part2kanban()
{
    ...
    foreach ($dolly as $item) {
        ...
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control dolly" name="dolly" value="' . $item->dolly . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control barcode" name="barcode" value="' . $item->barcode . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control pick_address" name="pick_address" value="' . $item->pick_address . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control delivery_address" name="delivery_address" value="' . $item->delivery_address . '">';
        echo '</td>';
        echo '<td>';
        echo '<input type="text" class="form-control delivery_address2" name="delivery_address2" value="' . $item->delivery_address2 . '">';
        echo '</td>';
        echo '<td>';
        ...
        echo '<button type="button" id="part2KanbanDelete" class="btn btn-danger btn-sm delete-part2kanban" value="' . $item->id . '">Delete</button>';
        echo '</td>';
        echo '</tr>';
    }
}
...
function get_kanban_id_by_name($post_data)
{
    ...
    // $status = $post_data['status'];
    $query = "SELECT id FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND kanban = '{$kanban}' AND cycle = {$cycle} limit 1";
    $result = $db->query($query);
...
{
    global $db, $tblStocking;
    $query = "SELECT sum(allocation*height) as total FROM {$tblStocking} WHERE area='System Fill'";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['total'] = $res['total'];

    $query = "select sum(A.booked_in - A.booked_out) as filled from scan_log as A inner join parts as B on UPPER(A.part) = UPPER(B.part_no) AND B.sf = 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
...
{
    global $db, $tblStocking;
    $query = "SELECT sum(allocation*height) as total FROM {$tblStocking} WHERE area='Part Stocking'";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['total'] = $res['total'];

    $query = "select sum(A.booked_in - A.booked_out) as filled from scan_log as A inner join parts as B on UPPER(A.part) = UPPER(B.part_no) AND B.ps = 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['filled'] = $res['filled'];
...
{
    global $db, $tblStocking;
    $query = "SELECT sum(allocation*height) as total FROM {$tblStocking} WHERE area='Free Location'";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['total'] = $res['total'];

    $query = "select sum(A.booked_in - A.booked_out) as filled from scan_log as A inner join parts as B on UPPER(A.part) = UPPER(B.part_no) AND B.fl = 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['filled'] = $res['filled'];
...
function get_all_session_json($post_data)
{
    ...
    $query = "SELECT count(distinct cycle) as remaining_cycles FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}' AND is_help=1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    ...
    $query = "SELECT MAX(cycle) as max_cycle FROM {$tblConveyancePicks} WHERE kanban_date = DATE('{$today}') AND is_help=1";
    ...
    $query = "SELECT COUNT(*) as incomplete_pick_count FROM {$tblConveyancePicks} WHERE kanban_date = DATE('{$today}') AND is_help=1 AND is_completed = 0 AND is_pick=1";
    ...
    $query = "SELECT COUNT(*) as incomplete_delivery_count FROM {$tblConveyancePicks} WHERE kanban_date = DATE('{$today}')  AND is_help=1 AND is_delivered = 0 AND is_pick=0";
    ...
    $query = "SELECT COUNT(*) as incomplete_stocking_count FROM {$tblHelpAlarm} WHERE DATE(clicked_time) = DATE('{$today}')  AND is_confirm=0 AND page ='Stocking'";
    ...
    $query = "SELECT COUNT(*) as incomplete_devan_count FROM {$tblHelpAlarm} WHERE DATE(clicked_time) = DATE('{$today}')  AND is_confirm=0 AND page ='Container Devan'";
    ...
    $query = "SELECT COUNT(*) as incomplete_driver_count FROM {$tblHelpAlarm} WHERE DATE(clicked_time = '{$today}')  AND is_confirm=0 AND page ='Driver'";
    ...
    $query = "SELECT COUNT(*) as complete_count FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}'  AND is_completed = 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
...
function get_pick_reason_chat_value($post_data)
{
    ...
    $query = "SELECT count(*) as count, t1.completed_reason as reason, t2.`name` FROM {$tblConveyancePicks} as t1 INNER JOIN {$tblReason} as t2 ON t1.completed_reason=t2.id AND t1.completed_reason <> 0 GROUP BY t1.completed_reason;";
    $result = $db->query($query);
    $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
{
    global $db, $tblConveyancePicks, $tblReason;
    $query = "SELECT count(*) as count, t1.delivered_reason as reason, t2.`name` FROM {$tblConveyancePicks} as t1 INNER JOIN {$tblReason} as t2 ON t1.delivered_reason=t2.id AND t1.delivered_reason <> 0 GROUP BY t1.delivered_reason;";
    $result = $db->query($query);
    $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
    $data['val'] = array_column($res, "count");
    $data['reason'] = array_column($res, "name");
...
function get_part_chat_value($post_data)
{
    ...
    $query = "select sum(booked_in-booked_out) as val, part FROM {$tblScanLog} GROUP BY part";
    $result = $db->query($query);
    $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
    // $res =  mysqli_fetch_array($result);
...
function save_tag($post_data)
{
    ...
    $db->query($query);
    $query = "INSERT INTO {$tblTag}  (`value`) value ('{$tag_value}')";
...
function save_opr_pick_delivery_setting($post_data)
{
    ...
    $db->query($query);
    $query = "INSERT INTO {$tblOPRSetting}  (`opr_type`, `value`) VALUES ('{$opr_type}', {$val})";
...
function get_andons_info_teamleader($post_data)
{
    ...
    $today = $post_data['today'];
    // $yesterday = $post_data['yesterday'];
    $query = "SELECT {$tblConveyancePicks}.kanban, {$tblConveyancePicks}.cycle, {$tblConveyancePicks}.helped_at, {$tblConveyancePicks}.deliveried_at, {$tblConveyancePicks}.picked_at,  {$tblReason}.name FROM {$tblConveyancePicks} LEFT JOIN {$tblReason} ON {$tblConveyancePicks}.completed_reason = {$tblReason}.id   WHERE ({$tblConveyancePicks}.kanban_date = '{$today}') AND  {$tblConveyancePicks}.is_help =1 AND {$tblConveyancePicks}.picked_at IS NOT NULL union SELECT {$tblConveyancePicks}.kanban, {$tblConveyancePicks}.cycle, {$tblConveyancePicks}.helped_at, {$tblConveyancePicks}.deliveried_at, {$tblConveyancePicks}.picked_at,  {$tblReason}.name FROM {$tblConveyancePicks} LEFT JOIN {$tblReason} ON {$tblConveyancePicks}.delivered_reason = {$tblReason}.id   WHERE ({$tblConveyancePicks}.kanban_date = '{$today}') AND  {$tblConveyancePicks}.is_help =1 AND {$tblConveyancePicks}.deliveried_at IS NOT NULL";
...
function get_remaining_incomplete_pick($post_data)
{
    ...
    $today = $post_data['today'];
    $yesterday = $post_data['yesterday'];
    $query = "SELECT count(distinct cycle) as remaining_cycles FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}' AND is_completed =0";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['remaining_cycles'] = $res['remaining_cycles'];

    // $query = "SELECT kanban, cycle, (SELECT max(cycle) from {$tblConveyancePicks}  WHERE is_help = 1 AND is_completed = 0  AND kanban_date = '{$today}') AS max_cycle from {$tblConveyancePicks} WHERE is_help = 1 AND is_completed = 0 AND kanban_date = '{$today}'";
    $query = "SELECT cp.kanban AS kanban, cp.cycle AS cycle, cp.helped_at AS helped_at, u.username AS username FROM {$tblConveyancePicks} AS cp INNER JOIN {$tblUsers} AS u ON cp.helped_user = u.ID AND cp.kanban_date >= '{$yesterday}' AND cp.kanban_date <= '{$today}' AND cp.is_help = 1 AND cp.is_completed =0";
    $result = $db->query($query);
    $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
    // $res =  mysqli_fetch_array($result);
    $data['incomplete_kanban'] = array_column($res, "kanban");
    ...
    $query = "SELECT MAX(cycle) as max_cycle FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}'";
    $result = $db->query($query);
...
function get_remaining_incomplete_pick_team_leader($post_data)
{
    ...
    $today = $post_data['today'];
    $yesterday = $post_data['yesterday'];
    $query = "SELECT count(distinct cycle) as remaining_cycles FROM {$tblConveyancePicks} WHERE kanban_date = '{$today}' AND is_help=1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['remaining_cycles'] = $res['remaining_cycles'];
    ...
    $query = "SELECT cp.id,cp.is_pick, cp.is_completed,cp.is_delivered, cp.kanban AS kanban, cp.cycle AS cycle, cp.helped_at AS helped_at, u.username AS username FROM {$tblConveyancePicks} AS cp INNER JOIN {$tblUsers} AS u ON cp.helped_user = u.ID AND cp.kanban_date = DATE('{$today}') AND cp.is_help = 1";
    $result = $db->query($query);
    $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
    // $res =  mysqli_fetch_array($result);
    $data['id'] = array_column($res, "id");
    ...
    $query = "SELECT COUNT(*) as complete_count FROM {$tblConveyancePicks} WHERE DATE(kanban_date) = DATE('{$today}')  AND is_completed = 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['complete_count'] = $res['complete_count'];
...
function get_sequence_control_addon($post_data)
{
    try {
        ...
        $query = "SELECT count(*) as count, cycle,sum(is_help) as is_help, sum(is_completed) as is_completed, sum(is_delivered) as is_delivered  FROM conveyance_picks WHERE kanban_date='{$today}' GROUP BY cycle ORDER BY cycle";
        $result = $db->query($query);
...
function get_remaining_build_delivery($post_data)
{
    ...
    $query = "SELECT count(distinct cycle) as remaining_cycles FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}' AND is_delivered =0";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['remaining_cycles'] = $res['remaining_cycles'];

    $query = "SELECT count(distinct cycle) as remaining_cycles_pick FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}' AND is_completed =0";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['remaining_cycles_pick'] = $res['remaining_cycles_pick'];

    $query = "SELECT count(distinct cycle) as remaining_cycles_delivery FROM {$tblConveyancePicks} WHERE kanban_date >= '{$yesterday}' AND kanban_date <= '{$today}' AND is_delivered =0";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
...
function get_last_upload_time_team_leader($post_data)
{
    ...
    $query = "SELECT CONCAT(`date`, ' ', `time`) AS `time` FROM {$tblContainerDevan} ORDER BY `date` DESC, `time` DESC LIMIT 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['devan_schedule'] = $res['time'];

    $query = "SELECT imported_at FROM {$tblConveyancePicks} ORDER BY imported_at DESC LIMIT 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
...
function get_opr_settings($post_data)
{
    ...
    $query = "SELECT `value` FROM {$tblOPRSetting} WHERE opr_type = 'pick' LIMIT 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
    $data['opr_pick_settings'] = $res['value'];

    $query = "SELECT `value` FROM {$tblOPRSetting} WHERE opr_type = 'delivery' LIMIT 1";
    $result = $db->query($query);
    $res =  mysqli_fetch_array($result);
...
function get_low_stocks($post_data)
{
    ...
    $query = "SELECT upper(t1.part_no) as part, SUM(t2.booked_in-t2.booked_out) AS count, t1.level_low, t1.level_medium FROM {$tblParts} AS t1 LEFT JOIN {$tblScanLog} AS t2 ON t1.part_no = t2.part GROUP BY t1.part_no";
...
function delete_periodData($post_data)
{
    ...
    $query = "DELETE FROM {$tblConveyancePicks} WHERE kanban_date BETWEEN '{$periodStart}' AND '{$periodEnd}'";
    $result = $db->query($query);
    $data = array();
    while ($row = mysqli_fetch_assoc($result)) {
        $data[] = $row;
...
{
    global $db, $tblExcelPick;
    $dayQuery = "SELECT * FROM {$tblExcelPick} WHERE (SUBSTRING(Module, 1, 2) = 'HB' OR SUBSTRING(Module, 1, 2) = 'HF') AND shift = 1 AND Stocking_Date = '{$post_data['date']}';";
    $nightQuery = "SELECT * FROM {$tblExcelPick} WHERE (SUBSTRING(Module, 1, 2) = 'HB' OR SUBSTRING(Module, 1, 2) = 'HF') AND shift = 2 AND Stocking_Date = '{$post_data['date']}';";
    $dayResult = $db->query($dayQuery);
    ...
    if ($dayResult->num_rows > 0) {
        ...
        for ($i = 0; $i < round(count($tmpArr) / 2); $i++) {
            if ($i < count($tmpArr) / 2) {
                ...
                $dayData = $dayData . "<script> modArr_day.push('" . $tmpArr[$i * 2]['Module'] . "') </script>";
            ...
            if ($i + 1 < round(count($tmpArr) / 2)) {
                $dayData = $dayData . "<script> contArr_day.push('" . $tmpArr[$i * 2 + 1]['Container'] . "') </script>";
    ...
    if ($nightResult->num_rows > 0) {
        ...
        for ($i = 0; $i <= floor(count($tmpArr) / 2); $i++) {
            ...
            <td rowspan='2' name='nightStWork'>
                ...
                <select disabled name='Fill Start' id='nightStSel_" . ($i + 1) . "' onchange='selectNightStName(" . ($i + 1) . ")' class='form-control'>
                    <option disabled selected value=''>Select User</option>
                </select>
            </td>
            <td rowspan='2' name='nightFnWork'>
                ...
                <select disabled name='Night Finish' id='nightFnSel_" . ($i + 1) . "' onchange='selectNightFnName(" . ($i + 1) . ")' class='form-control'>
                    <option disabled selected value=''>Select User</option>
                </select>
            </td>
            </tr>";
            ...
            if ($i + 1 < count($tmpArr) / 2) {
                $nightData = $nightData . "<script> contArr_night.push('" . $tmpArr[$i * 2 + 1]['Container'] . "') </script>";
        ...
        // Display a message if no rows were returned
        $nightData = $nightData . "No data found";
    }
    $data = array(
        "day" => $dayData,
        "night" => $nightData
