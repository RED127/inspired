    ...
    if ($max_cycle) {
        ...
        if ($kanban_id == 0) {
            if ($status == 'pick') {
                ...
                $error = '';
                if ($post_data['cycle'] == -1)
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_completed=0 AND kanban_date = '{$pick_date}' ORDER BY `is_help` ASC, `cycle` ASC, `is_completed` ASC, `imported_at` ASC LIMIT 1";
                else
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_completed=0 AND kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_completed` ASC, `imported_at` ASC LIMIT 1";
                ...
                $error = '';
                if ($post_data['cycle'] == -1)
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered=0 AND kanban_date = '{$pick_date}' ORDER BY `is_help` ASC, `cycle` ASC, `is_delivered` ASC, `imported_at` ASC";
                else
                    $query = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered=0 AND kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_delivered` ASC, `imported_at` ASC";
...
function make_pick_list_tr_first($pick, $status)
{
    ...
    echo '</td>';
    echo '<td style="text-align: center;">';
    echo '<button class="btn  select-kanban" style="width: 100px; height: 30px; background-color: ' . $pick['color'] . '"  data-kanban="' . $pick['id'] . '">'.$pick['dolly'].'</button>';
    echo '</td>';
    echo '</tr>';
}
...
function check_pick_finish($post_data)
{
    ...
    if ($status == 'pick') {
        ...
        if ($num == 0) {
            ...
            $query = "SELECT * FROM {$tblConveyancePicks} WHERE cycle={$post_data['cycle']} AND is_completed = 0 AND is_help = 1 AND kanban_date = '{$pick_date}'";
            $result = $db->query($query);
            $num = mysqli_num_rows($result);
            if ($num == 0)
                echo 'in_progress';
...
function confirm_conveyance_andon_help($post_data)
{
    ...
    if ($kanban_id) {
        ...
        if(isset($post_data['is_completed']))
            $query = "UPDATE {$tblConveyancePicks} SET is_help = 0, is_completed = {$post_data['is_completed']}, helped_user = {$helped_user} WHERE id = {$kanban_id}";
        if(isset($post_data['is_delivered']))
            $query = "UPDATE {$tblConveyancePicks} SET is_help = 0, is_delivered = {$post_data['is_delivered']}, helped_user = {$helped_user} WHERE id = {$kanban_id}";
        $result = $db->query($query);
        if ($result)
            echo 'ok';
        else
...
function save_part2kanban($post_data)
{
    ...
    if ($item_id == 0) {
        $query = "INSERT INTO {$tblPart2Kanban}  (`kanban`, `part_number`, `dolly`)
                    value ('{$kanban}', '{$part_number}', '{$dolly}')";
        $db->query($query);
        $insert_id = $db->insert_id;
        ...
        $query = "UPDATE {$tblPart2Kanban} SET `kanban` = '{$kanban}', `part_number` = '{$part_number}', `dolly` = '{$dolly}' WHERE `id` = {$item_id}";
        $result = $db->query($query);
        if ($result)
            echo 'Ok';
        else
